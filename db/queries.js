...
async function searchItem(itemName) {
  const { rows } = await pool.query(
    "SELECT item_name FROM items WHERE item_name=$1",
    [itemName]
  );
  return rows;
}
...
async function addNewOrder(itemId, categoryId, quantity) {
  await pool.query(
    "INSERT INTO orders (item_id,category_id,quantity) VALUES ($1,$2,$3)",
    [itemId, categoryId, quantity]
  );
}
...
async function selectOrderByItemIdAndCategoryId(itemId, categoryId) {
  const { rows } = await pool.query(
    "SELECT * FROM orders WHERE item_id=$1 AND category_id=$2",
    [itemId, categoryId]
  );
  return rows;
}
async function searchItemNameById(itemId) {
  const { rows } = await pool.query(
